...
def set_camera_angles(radius_min: float, radius_max: float, pitch_deg: float, yaw_deg_list: List[float]) -> List[Tuple[Vector, Euler]]:
    """Generate camera configurations at specified pitch and multiple yaw angles with random radius.

    Args:
        radius_min (float): Minimum radius from the object.
        radius_max (float): Maximum radius from the object.
        pitch_deg (float): Pitch angle in degrees.
        yaw_deg_list (List[float]): List of yaw angles in degrees.

    Returns:
        List[Tuple[Vector, Euler]]: List of camera positions and orientations.
    """
    ...
    for yaw_deg in yaw_deg_list:
        ...
        x = radius * np.cos(pitch_rad) * np.cos(yaw_rad)
        y = radius * np.cos(pitch_rad) * np.sin(yaw_rad)
        ...
        camera_configs.append((position, orientation))
...
def enable_depth_pass(scene: bpy.types.Scene, depth_output_dir: str) -> None:
    """ Enable depth reading 
    Enable a Z-depth pass in Blender, and set up a File Output node so the depth 
    is saved into an EXR file in `depth_output_dir`.
    """
    # 1) Turn on use_nodes and the Z pass
    scene.use_nodes = True
    scene.view_layers["ViewLayer"].use_pass_z = True

    # 2) Clear any default or existing compositor nodes
    node_tree = scene.node_tree
    for node in node_tree.nodes:
        node_tree.nodes.remove(node)

    # 3) Create a Render Layers node to access the Z pass
    render_layers_node = node_tree.nodes.new("CompositorNodeRLayers")
    render_layers_node.location = (0, 0)

    # 4) Create a File Output node to save the depth image
    file_output_node = node_tree.nodes.new("CompositorNodeOutputFile")
    file_output_node.name = "Depth Output"
    file_output_node.label = "Depth Output"
    file_output_node.location = (300, 0)

    # Let’s store files in the provided directory
    file_output_node.base_path = depth_output_dir

    # By default there is a single input "Image", we will connect "Z" pass to it
    # But let's rename it for clarity:
    file_output_node.file_slots[0].use_node_format = True
    file_output_node.file_slots[0].path = "depth_"  # base name, we can modify if needed
    file_output_node.format.file_format = "OPEN_EXR"
    file_output_node.format.color_depth = "32"  # 32-bit float
    file_output_node.format.color_mode = "BW"   # single channel is enough for Z

    # 5) Connect the Z output from Render Layers to the File Output node
    node_tree.links.new(render_layers_node.outputs["Depth"], file_output_node.inputs[0])
...
def get_3x3_K_matrix_from_blender(
    camera_obj: bpy.types.Object, scene: bpy.types.Scene
) -> np.ndarray:
    """
    Computes the 3x3 intrinsic camera matrix K for the given camera.
    ...
    # 2. Focal length (mm) and sensor width (mm)
    f_mm = cam_data.lens
    sensor_width_mm = cam_data.sensor_width

    # 3. Principal point. By default, we assume center of the image
    cx = w / 2.0
    cy = h / 2.0

    # 4. Focal length in pixel units: (focal_mm / sensor_width_mm) * image_width_pixels
    # If the sensor is auto-fitting horizontally, we compute fx as:
    fx = (f_mm / sensor_width_mm) * w
    # Usually, for square pixels, fy = fx * (aspect_ratio_correction) 
    # but if there's no significant difference in sensor height, you can do:
    fy = fx

    # Build the 3x3 K matrix
    K = np.array([
        [fx,  0,  cx],
        [ 0, fy,  cy],
        [ 0,  0,   1],
...
def render_object(
    ...
    # Simply move the empty up by half the height of the bbox
    bbox_min, bbox_max = scene_bbox()
    height = bbox_max.z - bbox_min.z
    # Move the empty up to compensate for the object positioning
    empty.location.z += height/2

    # randomize the lighting
    randomize_lighting()

    # enable depth
    enable_depth_pass(scene, depth_output_dir=output_dir)
    ...
    # render with pre-set sequence of cam angles
    camera_configs = []
    camera_configs.extend(set_camera_angles(radius_min=2.4, radius_max=2.8, pitch_deg=30, yaw_deg_list=[30, 75, 120, 165, 210, 255, 300, 345]))
    camera_configs.extend(set_camera_angles(radius_min=2.4, radius_max=2.8, pitch_deg=0, yaw_deg_list=[60, 105, 150, 195, 240, 285, 330, 375]))

    for i, (position, orientation) in enumerate(camera_configs):
        camera = bpy.data.objects["Camera"]
        camera.location = position
        camera.rotation_euler = orientation

        # Render the scene from this camera configuration
        render_path = os.path.join(output_dir, f"{i:03d}.png")
        bpy.context.scene.render.filepath = render_path

        file_output_node = bpy.context.scene.node_tree.nodes["Depth Output"]
        file_output_node.file_slots[0].path = f"depth_{i:03d}"
    ...
    K = get_3x3_K_matrix_from_blender(camera, scene)
    K_path = os.path.join(output_dir, f"cam_K.npy")
    np.save(K_path, K)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the data root, which contains obj_models and render folders",
    )
    parser.add_argument(
        "--engine",
        type=str,
    ...
    # render category
    obj_models_root = Path(args.data_root) / "obj_models"
    categories = os.listdir(obj_models_root)
    categories = [c for c in categories if os.path.isdir(os.path.join(obj_models_root, c))]
    render_root= Path(args.data_root) / "render"

    for category in categories:
        category_render_path = os.path.join(render_root, category)
        os.makedirs(category_render_path, exist_ok=True)

        category_obj_path = os.path.join(obj_models_root, category)
        for obj_file in os.listdir(category_obj_path):
            if not obj_file.endswith(".glb"):
                continue
            obj_path = os.path.join(category_obj_path, obj_file)
            obj_idx = obj_file.split(".")[0]
            output_dir = os.path.join(category_render_path, obj_idx)
            os.makedirs(output_dir, exist_ok=True)
